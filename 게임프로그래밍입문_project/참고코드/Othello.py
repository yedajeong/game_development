...
class HUD(cocos.layer.Layer):
    ...
    def show_game_over(self, winner):
        w, h = cocos.director.director.get_window_size()
        game_over = cocos.text.Label(winner, font_size=50,anchor_x='center',
                                    anchor_y='center', color=(50, 50, 255, 255))
...
class GameLayer(cocos.layer.Layer):
    ...
    def __init__(self, difficulty, hud_layer):
        ...
        #easy: depth2+돌개수 / normal: depth4+돌개수 / hard: depth4+weigthSum
    ...
    def minimax(self, player):
        ...
        for i, move in enumerate(moves):
            ...
            #scores[i] = self.boardScore(boardCopy)
                scores.append(self.boardScore(boardCopy))
            else:
                #scores[i] = self.minMove(boardCopy, 2, alpha, beta)
                scores.append(self.minMove(boardCopy, 2, alpha, beta))
            if scores[i] > alpha:
                alpha = scores[i]
            if beta <= alpha:
                break
    ...
    def boardScore(self, board):
        if self.difficulty == 0 or self.difficulty == 1: #easy or normal
            computerNum = 0
            personNum = 0
            for y in range(0, self.row):
                for x in range(0, self.column):
